...
#𝐿 = 𝜏(𝑌 – 1991) + {(𝜇 + 𝜃𝑖𝑁 + 𝐶)𝜅}(𝑃 𝑆)p
   ...
   L_nuar <- L*(1 - EEAFdato - EMA - ETS)(1- EPJ)

   return(list(L, L_nuar))
}
...
N_func <- function(
  ...
  beta_udb = 0.016314,  # Mineral N from grazing in harvest year
  beta_f0 = 0.038245,   # Nitrogen fixation in harvest year
  ...
  beta_f1 = 0.025499,    # Nitrogen fixation in previous/pre-previous crop (F1+F2)
...
# 1. 2. Define Default Parameter Values (Beta Coefficients)
...
C_func <- function(M, W, MP, WP) {
  ...
  M_p <- c(
    ...
    M7 = -0.640,         # Sukkerroer og foderroer (Sugar beets and fodder beets)
    M8 = 3.534,          # Majshelsæd og kartofler (Maize silage and potatoes)
    M9 = -7.319,         # Vinterraps (Winter rapeseed)
    M10 = -1.248,        # Vintersæd efter græs, kløvergræs, frøgræs og brak (Winter cereals after grass, clover, seed grass, or fallow)
    M11 = 19.524,        # Majshelsæd efter græs, kløvergræs, frøgræs og brak (Maize silage after grass, clover, seed grass, or fallow)
    M12 = -6.229,        # Vårsæd efter græs, kløvergræs, frøgræs og brak (Spring cereals after grass, clover, seed grass, or fallow)
    M13 = -2.866         # Bælgsæd og vårraps (Legumes and spring rapeseed)
  )
  ...
  W_p <- c(
    ...
    W7 = -1.049,         # Vintersæd efter græs og frøgræs (Winter cereals after grass and seed grass)
    W8 = -21.060         # Græs og kløvergræs pløjet sent efterår eller vinter (Grass and clover plowed late autumn or winter)
  )
...
# P component ----
P_func <- function(jbnr, AAA, ASM, ASMFfr) {

    # Parameters for sandy soil (JB1 + JB3)
    PAAS <- 0.001194     # April–August runoff on sandy soil
    PSMS <- 0.001107     # Sept–March runoff on sandy soil
    PSMSFfr <- 0.000856  # Sept–March runoff in preceding year on sandy soil

    # Parameters for loamy soil (all other JB types)
    PAAM <- 0.000798     # April–August runoff on loamy soil
    PSMM <- 0.000745     # Sept–March runoff on loamy soil
    PSMMFfr <- 0.000638  # Sept–March runoff in preceding year on loamy soil

    # Compute P based on soil type
    if (jbnr<=3) {
      # Sandy soil formula
      P <- (1 - exp(-PAAS * AAA - PSMS * ASM)) * exp(-PSMSFfr * ASMFfr)
    } else {
      # Loamy soil formula
      P <- (1 - exp(-PAAM * AAA - PSMM * ASM)) * exp(-PSMMFfr * ASMFfr)
    }
